...
std::vector<unsigned int> generate_random()
{
    std::random_device rd;
    auto generator = std::mt19937 { rd() };
    auto dist = std::uniform_int_distribution<> { 1, 99 };
    std::vector<unsigned int> numbers(100000000, 0);
...
void execute()
{
    ...
    // use member functions: load, store and exchange
    {
        // load and store
    ...
    // use member functions: fetch_add and fetch_sub
    {
        const int thread_count = 10;
        std::atomic<unsigned long> sum { 0 };
        std::vector<unsigned int> numbers = generate_random();
        ...
        for (int i = 0; i < thread_count; ++i) {
            ...
            }, i * (size / thread_count), (i + 1) * (size / thread_count));
        }
    ...
    {
        std::atomic_flag lock = ATOMIC_FLAG_INIT;  // deprecated in C++20
        int counter = 0;
    ...
    // efficient thread-synchronization mechanism provided in C++20
    {
